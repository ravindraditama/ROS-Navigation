...
Motor right(10,11,20,21); //dir 1,2  en 1, 2
Motor left(8,9,18,19);

volatile long encoder0Pos = 0;    // encoder 1
volatile long encoder1Pos = 0;    // encoder 2

double left_kp = 1.8 , left_ki = 18.2, left_kd = 0;             //kiri p= 3.8;
double right_kp = 1.8 , right_ki = 18.2, right_kd = 0;
...
//  Serial.begin(115200);
  ...
  rightPID.SetOutputLimits(-254, 254);

  leftPID.SetMode(AUTOMATIC);
  leftPID.SetSampleTime(1);
  leftPID.SetOutputLimits(-254, 254);
...
void loop() {
  ...
  if (currentMillis - prevMillis >= LOOPTIME){
    ...
    demand_speed_left = demandx - (demandz*0.1075); //demanz*0.1075
    ...
    speed_act_left = encoder0Diff/59;                    
    speed_act_right = encoder1Diff/59; 
  
    encoder0Error = (demand_speed_left*59)-encoder0Diff; // 3965 ticks in 1m = 39.65 ticks in 10ms, due to the 10 millis loop
    encoder1Error = (demand_speed_right*59)-encoder1Diff;
  
    encoder0Prev = encoder0Pos; // Saving values
    encoder1Prev = encoder1Pos;
  
    left_setpoint = demand_speed_left*54;  //Setting required speed as a mul/frac of 1 m/s
    right_setpoint = demand_speed_right*54;
